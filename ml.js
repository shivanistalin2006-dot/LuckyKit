...
function handleClick(item, el) {
  ...
  if (item === expected) {
    ...
    if (state.lives <= 0) {
      ...
      if (window.ArcadeCore && state.score > 0) window.ArcadeCore.addXP(Math.floor(state.score / 10));
...
function bindUI() {
  ...
  if ($("homeFromHeroBtn")) $("homeFromHeroBtn").addEventListener("click", () => window.location.href = "index.html");
