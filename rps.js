...
function revealResults(){
    ...
    // Collect all unique moves played this round
    let playedMoves = new Set(Object.values(moves));

    if (playedMoves.size === 1 || playedMoves.size === 3) {
        // If everyone played the same, or all 3 moves were played, it's a draw
        resultHTML += "<h3>🤝 Draw Round</h3>";
    } else {
        // Only 2 distinct moves were played. Find which one beats the other.
        players.forEach(player => {
            let winCount = 0;
            players.forEach(opponent => {
                if (player !== opponent && beats(moves[player], moves[opponent])) {
                    winCount++;
                }
            });
            // If they beat anyone and it's not a circular draw, they are a winner
            if (winCount > 0) {
                winners.push(player);
            }
        });
        ...
        if (winners.length > 0) {
            resultHTML += "<h3>🏆 Round Winners</h3>";
            winners.forEach(player => {
                scores[player]++;
                document.getElementById(`score-${player}`).textContent = scores[player];
                resultHTML += `<p>${player}</p>`;
            });
        }
    }
